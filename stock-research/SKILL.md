...
2. 调用 `sec-edgar-mcp.health_check()`，验证 user_agent_configured + server_version
   在 `>=0.2,<0.3`
2.5. 验证 sec-edgar UA 是否为真实可联系邮箱（**非** noreply）。读
   `health_check()` 返回的 `user_agent` 字段，如果包含字符串
   `users.noreply.github.com` 或为空 → STOP，告诉用户必须把
   `sec-edgar-mcp/.env` 里的 `SEC_EDGAR_USER_AGENT` 改为真实可达邮箱
   （SEC fair-use 政策要求；`user_agent_configured=true` 只验本地
   env 是否非空，不验 SEC 端是否实际接受，2026-05-25 PB-3 实测发现
   noreply 邮箱触发 SEC 403 deny，记为 R7）。
